...
Now we can start the Remote Catalog app under `mf_catalog` with `npm run dev` to expose the Manifest entry file and optionally load the site under `http://localhost:3001`:
