...
In the previous [Part 2](/blog/2025/microfrontends-part-2-injecting-federated-components-into-sitecore-apps) we exposed some Remote Product components into our Sitecore Next.js app. 

Today in Part 3, we will dive deeper into Remote Components by creating a Product Detail component and PDP Service in Remote that uses the Sitecore component-level data fetching in the Host App, along with a Sitecore wildcard for the PDP Detail page.
...
> In Part 3 of this series, we will dive deeper into Remote Components and leveraging the Sitecore Dynamic Page Route inside of other Host apps without needing to do any Sitecore specific coding in those apps.
